...
function parse$1(str) {
    ...
    while (state.pos < state.str.length) {
        ...
        var keyword = parseKeyword(state);
        if (keyword) {
            if (keyword in parsers$1) {
                parsers$1[keyword](state, model);
            }
            else {
...
var NONE$1 = -1;
var AnimVectorType$1;
(function (AnimVectorType) {
    AnimVectorType[AnimVectorType["INT1"] = 0] = "INT1";
    AnimVectorType[AnimVectorType["FLOAT1"] = 1] = "FLOAT1";
    AnimVectorType[AnimVectorType["FLOAT3"] = 2] = "FLOAT3";
    AnimVectorType[AnimVectorType["FLOAT4"] = 3] = "FLOAT4";
})(AnimVectorType$1 || (AnimVectorType$1 = {}));
var animVectorSize$1 = (_a$1 = {},
    _a$1[AnimVectorType$1.INT1] = 1,
    _a$1[AnimVectorType$1.FLOAT1] = 1,
    _a$1[AnimVectorType$1.FLOAT3] = 3,
    _a$1[AnimVectorType$1.FLOAT4] = 4,
    _a$1);
var State = /** @class */ (function () {
    ...
    State.prototype.uint16 = function () {
        var res = this.view.getUint16(this.pos, BIG_ENDIAN$1);
        this.pos += 2;
        return res;
    };
    State.prototype.int32 = function () {
        var res = this.view.getInt32(this.pos, BIG_ENDIAN$1);
        this.pos += 4;
        return res;
    };
    State.prototype.float32 = function () {
        var res = this.view.getFloat32(this.pos, BIG_ENDIAN$1);
        this.pos += 4;
        return res;
    };
    ...
    State.prototype.animVector = function (type) {
        ...
        var isInt = type === AnimVectorType$1.INT1;
        var vectorSize = animVectorSize$1[type];
        var keysCount = this.int32();
        res.LineType = this.int32();
        res.GlobalSeqId = this.int32();
        if (res.GlobalSeqId === NONE$1) {
    ...
    while (state.pos < startPos + size) {
        ...
        for (var i = 0; i < layersCount; ++i) {
            ...
            while (state.pos < startPos2 + size2) {
                ...
                if (keyword === 'KMTA') {
                    layer.Alpha = state.animVector(AnimVectorType$1.FLOAT1);
                }
                else if (keyword === 'KMTF') {
                    layer.TextureID = state.animVector(AnimVectorType$1.INT1);
                }
                else {
...
var MODEL_TEXTURE_PATH_LENGTH$1 = 0x100;
...
function parseGeosetAnims(model, state, size) {
    ...
    while (state.pos < startPos + size) {
        ...
        if (geosetAnim.GeosetId === NONE$1) {
            geosetAnim.GeosetId = null;
        }
        while (state.pos < animStartPos + animSize) {
            var keyword = state.keyword();
            if (keyword === 'KGAO') {
                geosetAnim.Alpha = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KGAC') {
                geosetAnim.Color = state.animVector(AnimVectorType$1.FLOAT3);
            }
            else {
...
var MODEL_NODE_NAME_LENGTH$1 = 0x50;
function parseNode(model, node, state) {
    ...
    if (node.ObjectId === NONE$1) {
        node.ObjectId = null;
    }
    node.Parent = state.int32();
    if (node.Parent === NONE$1) {
        node.Parent = null;
    }
    ...
    while (state.pos < startPos + size) {
        var keyword = state.keyword();
        if (keyword === 'KGTR') {
            node.Translation = state.animVector(AnimVectorType$1.FLOAT3);
        }
        else if (keyword === 'KGRT') {
            node.Rotation = state.animVector(AnimVectorType$1.FLOAT4);
        }
        else if (keyword === 'KGSC') {
            node.Scaling = state.animVector(AnimVectorType$1.FLOAT3);
        }
        else {
...
function parseBones(model, state, size) {
    ...
    while (state.pos < startPos + size) {
        ...
        if (bone.GeosetId === NONE$1) {
            bone.GeosetId = null;
        }
        bone.GeosetAnimId = state.int32();
        if (bone.GeosetAnimId === NONE$1) {
...
var MODEL_ATTACHMENT_PATH_LENGTH$1 = 0x100;
...
var MODEL_PARTICLE_EMITTER_PATH_LENGTH$1 = 0x100;
function parseParticleEmitters(model, state, size) {
    ...
    while (state.pos < startPos + size) {
        ...
        while (state.pos < emitterStart + emitterSize) {
            var keyword = state.keyword();
            if (keyword === 'KPEV') {
                emitter.Visibility = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KPEE') {
                emitter.EmissionRate = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KPEG') {
                emitter.Gravity = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KPLN') {
                emitter.Longitude = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KPLT') {
                emitter.Latitude = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KPEL') {
                emitter.LifeSpan = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KPES') {
                emitter.InitVelocity = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else {
    ...
    while (state.pos < startPos + size) {
        ...
        while (state.pos < emitterStart + emitterSize) {
            ...
            if (keyword === 'KP2V') {
                ...
                emitter.Width = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KP2N') {
                emitter.Length = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KP2S') {
                emitter.Speed = state.animVector(AnimVectorType$1.FLOAT1);
            }
                ...
                emitter.Variation = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else {
...
var MODEL_CAMERA_NAME_LENGTH$1 = 0x50;
function parseCameras(model, state, size) {
    ...
    while (state.pos < startPos + size) {
        ...
        while (state.pos < cameraStart + cameraSize) {
            var keyword = state.keyword();
            if (keyword === 'KCTR') {
                camera.Translation = state.animVector(AnimVectorType$1.FLOAT3);
            }
            else if (keyword === 'KTTR') {
                camera.TargetTranslation = state.animVector(AnimVectorType$1.FLOAT3);
            }
            else if (keyword === 'KCRL') {
                camera.Rotation = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else {
...
function parseLights(model, state, size) {
    ...
    while (state.pos < startPos + size) {
        ...
        while (state.pos < lightStart + lightSize) {
            var keyword = state.keyword();
            if (keyword === 'KLAV') {
                light.Visibility = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KLAC') {
                light.Color = state.animVector(AnimVectorType$1.FLOAT3);
            }
            else if (keyword === 'KLAI') {
                light.Intensity = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KLBC') {
                light.AmbColor = state.animVector(AnimVectorType$1.FLOAT3);
            }
            else if (keyword === 'KLBI') {
                light.AmbIntensity = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KLAS') {
                light.AttenuationStart = state.animVector(AnimVectorType$1.INT1);
            }
            else if (keyword === 'KLAE') {
                light.AttenuationEnd = state.animVector(AnimVectorType$1.INT1);
            }
            else {
...
function parseTextureAnims(model, state, size) {
    ...
    while (state.pos < startPos + size) {
        ...
        while (state.pos < animStart + animSize) {
            var keyword = state.keyword();
            if (keyword === 'KTAT') {
                anim.Translation = state.animVector(AnimVectorType$1.FLOAT3);
            }
            else if (keyword === 'KTAR') {
                anim.Rotation = state.animVector(AnimVectorType$1.FLOAT4);
            }
            else if (keyword === 'KTAS') {
                anim.Scaling = state.animVector(AnimVectorType$1.FLOAT3);
            }
            else {
                throw new Error('Incorrect light chunk data ' + keyword);
            }
    ...
    while (state.pos < startPos + size) {
        ...
        while (state.pos < emitterStart + emitterSize) {
            ...
            if (keyword === 'KRVS') {
                ...
                emitter.HeightAbove = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KRHB') {
                emitter.HeightBelow = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KRAL') {
                emitter.Alpha = state.animVector(AnimVectorType$1.FLOAT1);
            }
            else if (keyword === 'KRTX') {
                emitter.TextureSlot = state.animVector(AnimVectorType$1.INT1);
            }
            else {
...
function generate$1(model) {
    var res = '';
    for (var _i = 0, generators_1 = generators$1; _i < generators_1.length; _i++) {
...
var blpimage = /*#__PURE__*/Object.freeze({
    __proto__: null,
    get BLPType () { return BLPType; },
    get BLPContent () { return BLPContent; }
});
...
var decoder = function decode (data) {
...
function interpVec3(out, frame, left, right, lineType) {
    ...
    if (lineType === LineType.DontInterp) {
        ...
        return glMatrix.vec3.bezier(out, left.Vector, left.OutTan, right.InTan, right.Vector, t);
    }
    else if (lineType === LineType.Hermite) {
        return glMatrix.vec3.hermite(out, left.Vector, left.OutTan, right.InTan, right.Vector, t);
    }
    else {
        return glMatrix.vec3.lerp(out, left.Vector, right.Vector, t);
    }
}
function interpQuat(out, frame, left, right, lineType) {
    ...
    if (lineType === LineType.DontInterp) {
        ...
        return glMatrix.quat.sqlerp(out, left.Vector, left.OutTan, right.InTan, right.Vector, t);
    }
    else {
        return glMatrix.quat.slerp(out, left.Vector, right.Vector, t);
    }
}
...
var rotateCenter = glMatrix.vec3.fromValues(0, 0, 0);
var firstColor = glMatrix.vec4.create();
var secondColor = glMatrix.vec4.create();
var color = glMatrix.vec4.create();
var tailPos = glMatrix.vec3.create();
var tailCross = glMatrix.vec3.create();
...
var ParticlesController = /** @class */ (function () {
    ...
    ParticlesController.prototype.initShaders = function () {
        var vertex = this.vertexShader = getShader(this.gl, vertexShader$1, this.gl.VERTEX_SHADER);
        var fragment = this.fragmentShader = getShader(this.gl, fragmentShader$2, this.gl.FRAGMENT_SHADER);
        ...
        if (emitter.particles.length) {
            ...
            if (emitter.type & ParticleEmitter2FramesFlags.Head) {
                if (emitter.props.Flags & ParticleEmitter2Flags.XYQuad) {
                    glMatrix.vec3.set(this.particleBaseVectors[0], -1, 1, 0);
                    glMatrix.vec3.set(this.particleBaseVectors[1], -1, -1, 0);
                    glMatrix.vec3.set(this.particleBaseVectors[2], 1, 1, 0);
                    glMatrix.vec3.set(this.particleBaseVectors[3], 1, -1, 0);
                }
                else {
                    glMatrix.vec3.set(this.particleBaseVectors[0], 0, -1, 1);
                    glMatrix.vec3.set(this.particleBaseVectors[1], 0, -1, -1);
                    glMatrix.vec3.set(this.particleBaseVectors[2], 0, 1, 1);
                    glMatrix.vec3.set(this.particleBaseVectors[3], 0, 1, -1);
                    for (var i = 0; i < 4; ++i) {
                        glMatrix.vec3.transformQuat(this.particleBaseVectors[i], this.particleBaseVectors[i], this.rendererData.cameraQuat);
                    }
                }
            }
    ...
    ParticlesController.prototype.createParticle = function (emitter, emitterMatrix) {
        ...
        var latitude = degToRad(this.interp.animVectorVal(emitter.props.Latitude, 0));
        ...
        glMatrix.vec3.transformMat4(particle.pos, particle.pos, emitterMatrix);
        if (variation > 0) {
            speedScale *= 1 + rand(-variation, variation);
        }
        glMatrix.vec3.set(particle.speed, 0, 0, speedScale);
        particle.angle = rand(0, Math.PI * 2);
        glMatrix.vec3.rotateY(particle.speed, particle.speed, rotateCenter, rand(0, latitude));
        glMatrix.vec3.rotateZ(particle.speed, particle.speed, rotateCenter, particle.angle);
        if (emitter.props.Flags & ParticleEmitter2Flags.LineEmitter) {
            particle.speed[0] = 0;
        }
        glMatrix.vec3.transformMat4(particle.speed, particle.speed, emitterMatrix);
    ...
    ParticlesController.prototype.updateParticleVertices = function (particle, index, emitter, firstHalf, t) {
        ...
        if (emitter.type & ParticleEmitter2FramesFlags.Tail) {
            ...
            glMatrix.vec3.cross(tailCross, particle.speed, this.rendererData.cameraPos);
            glMatrix.vec3.normalize(tailCross, tailCross);
            glMatrix.vec3.scale(tailCross, tailCross, scale);
    ...
    ParticlesController.prototype.updateParticleTexCoordsByType = function (index, emitter, firstHalf, t, type) {
        ...
        var frame = Math.round(lerp(firstFrame, secondFrame, t));
    ...
    ParticlesController.prototype.updateParticleColor = function (index, emitter, firstHalf, t) {
        ...
        glMatrix.vec4.lerp(color, firstColor, secondColor, t);
        for (var i = 0; i < 4; ++i) {
...
var RibbonsController = /** @class */ (function () {
    ...
    RibbonsController.prototype.initShaders = function () {
        var vertex = this.vertexShader = getShader(this.gl, vertexShader, this.gl.VERTEX_SHADER);
        var fragment = this.fragmentShader = getShader(this.gl, fragmentShader$1, this.gl.FRAGMENT_SHADER);
        var shaderProgram = this.shaderProgram = this.gl.createProgram();
        this.gl.attachShader(shaderProgram, vertex);
        this.gl.attachShader(shaderProgram, fragment);
        this.gl.linkProgram(shaderProgram);
    ...
    RibbonsController.prototype.appendVertices = function (emitter) {
        var first = glMatrix.vec3.clone(emitter.props.PivotPoint);
        var second = glMatrix.vec3.clone(emitter.props.PivotPoint);
        ...
        glMatrix.vec3.transformMat4(first, first, emitterMatrix);
        glMatrix.vec3.transformMat4(second, second, emitterMatrix);
...
var scaling = glMatrix.vec3.create();
var defaultTranslation = glMatrix.vec3.fromValues(0, 0, 0);
var defaultRotation = glMatrix.quat.fromValues(0, 0, 0, 1);
var defaultScaling = glMatrix.vec3.fromValues(1, 1, 1);
var tempParentRotationQuat = glMatrix.quat.create();
var tempParentRotationMat = glMatrix.mat4.create();
var tempCameraMat = glMatrix.mat4.create();
var tempTransformedPivotPoint = glMatrix.vec3.create();
var tempAxis = glMatrix.vec3.create();
var tempLockQuat = glMatrix.quat.create();
var tempLockMat = glMatrix.mat4.create();
var tempXAxis = glMatrix.vec3.create();
var tempCameraVec = glMatrix.vec3.create();
var tempCross0 = glMatrix.vec3.create();
var tempCross1 = glMatrix.vec3.create();
var tempPos = glMatrix.vec3.create();
var tempSum = glMatrix.vec3.create();
var tempVec3 = glMatrix.vec3.create();
var identifyMat3 = glMatrix.mat3.create();
var texCoordMat4 = glMatrix.mat4.create();
var texCoordMat3 = glMatrix.mat3.create();
var ModelRenderer = /** @class */ (function () {
    function ModelRenderer(model) {
        ...
        this.rendererData.teamColor = glMatrix.vec3.fromValues(1., 0., 0.);
        this.rendererData.cameraPos = glMatrix.vec3.create();
        this.rendererData.cameraQuat = glMatrix.quat.create();
        this.setSequence(0);
    ...
    ModelRenderer.prototype.setCamera = function (cameraPos, cameraQuat) {
        glMatrix.vec3.copy(this.rendererData.cameraPos, cameraPos);
        glMatrix.quat.copy(this.rendererData.cameraQuat, cameraQuat);
    };
    ...
    ModelRenderer.prototype.setTeamColor = function (color) {
        glMatrix.vec3.copy(this.rendererData.teamColor, color);
    };
    ...
    ModelRenderer.prototype.renderSkeleton = function (mvMatrix, pMatrix, nodes) {
        ...
        var line = function (node0, node1) {
            glMatrix.vec3.transformMat4(tempPos, node0.node.PivotPoint, node0.matrix);
            coords.push(tempPos[0], tempPos[1], tempPos[2]);
            glMatrix.vec3.transformMat4(tempPos, node1.node.PivotPoint, node1.matrix);
            coords.push(tempPos[0], tempPos[1], tempPos[2]);
    ...
    ModelRenderer.prototype.initSkeletonShaderProgram = function () {
        var vertex = this.skeletonVertexShader = getShader(this.gl, skeletonVertexShader, this.gl.VERTEX_SHADER);
        var fragment = this.skeletonFragmentShader = getShader(this.gl, skeletonFragmentShader, this.gl.FRAGMENT_SHADER);
    ...
    ModelRenderer.prototype.generateGeosetVertices = function (geosetIndex) {
        ...
        for (var i = 0; i < buffer.length; i += 3) {
            ...
            glMatrix.vec3.set(tempPos, geoset.Vertices[i], geoset.Vertices[i + 1], geoset.Vertices[i + 2]);
            glMatrix.vec3.set(tempSum, 0, 0, 0);
            for (var j = 0; j < group.length; ++j) {
                glMatrix.vec3.add(tempSum, tempSum, glMatrix.vec3.transformMat4(tempVec3, tempPos, this.rendererData.nodes[group[j]].matrix));
            }
            glMatrix.vec3.scale(tempPos, tempSum, 1 / group.length);
            buffer[i] = tempPos[0];
    ...
    ModelRenderer.prototype.initShaders = function () {
        ...
        var vertex = this.vertexShader = getShader(this.gl, vertexShaderSource, this.gl.VERTEX_SHADER);
        var fragment = this.fragmentShader = getShader(this.gl, fragmentShader, this.gl.FRAGMENT_SHADER);
        var shaderProgram = this.shaderProgram = this.gl.createProgram();
        this.gl.attachShader(shaderProgram, vertex);
        this.gl.attachShader(shaderProgram, fragment);
        this.gl.linkProgram(shaderProgram);
    ...
    ModelRenderer.prototype.updateNode = function (node) {
        ...
        if (!translationRes && !rotationRes && !scalingRes) {
            glMatrix.mat4.identity(node.matrix);
        }
        else if (translationRes && !rotationRes && !scalingRes) {
            glMatrix.mat4.fromTranslation(node.matrix, translationRes);
        }
        else if (!translationRes && rotationRes && !scalingRes) {
            mat4fromRotationOrigin(node.matrix, rotationRes, node.node.PivotPoint);
        }
        else {
            glMatrix.mat4.fromRotationTranslationScaleOrigin(node.matrix, rotationRes || defaultRotation, translationRes || defaultTranslation, scalingRes || defaultScaling, node.node.PivotPoint);
        }
        if (node.node.Parent || node.node.Parent === 0) {
            glMatrix.mat4.mul(node.matrix, this.rendererData.nodes[node.node.Parent].matrix, node.matrix);
        }
        var billboardedLock = node.node.Flags & NodeFlags.BillboardedLockX ||
        ...
        if (node.node.Flags & NodeFlags.Billboarded) {
            glMatrix.vec3.transformMat4(tempTransformedPivotPoint, node.node.PivotPoint, node.matrix);
            if (node.node.Parent || node.node.Parent === 0) {
                // cancel parent rotation from PivotPoint
                glMatrix.mat4.getRotation(tempParentRotationQuat, this.rendererData.nodes[node.node.Parent].matrix);
                glMatrix.quat.invert(tempParentRotationQuat, tempParentRotationQuat);
                mat4fromRotationOrigin(tempParentRotationMat, tempParentRotationQuat, tempTransformedPivotPoint);
                glMatrix.mat4.mul(node.matrix, tempParentRotationMat, node.matrix);
            }
            // rotate to camera
            mat4fromRotationOrigin(tempCameraMat, this.rendererData.cameraQuat, tempTransformedPivotPoint);
            glMatrix.mat4.mul(node.matrix, tempCameraMat, node.matrix);
        }
        else if (billboardedLock) {
            glMatrix.vec3.transformMat4(tempTransformedPivotPoint, node.node.PivotPoint, node.matrix);
            glMatrix.vec3.copy(tempAxis, node.node.PivotPoint);
            ...
            glMatrix.vec3.transformMat4(tempAxis, tempAxis, node.matrix);
            glMatrix.vec3.sub(tempAxis, tempAxis, tempTransformedPivotPoint);
            glMatrix.vec3.set(tempXAxis, 1, 0, 0);
            glMatrix.vec3.add(tempXAxis, tempXAxis, node.node.PivotPoint);
            glMatrix.vec3.transformMat4(tempXAxis, tempXAxis, node.matrix);
            glMatrix.vec3.sub(tempXAxis, tempXAxis, tempTransformedPivotPoint);
            glMatrix.vec3.set(tempCameraVec, -1, 0, 0);
            glMatrix.vec3.transformQuat(tempCameraVec, tempCameraVec, this.rendererData.cameraQuat);
            glMatrix.vec3.cross(tempCross0, tempAxis, tempCameraVec);
            glMatrix.vec3.cross(tempCross1, tempAxis, tempCross0);
            glMatrix.vec3.normalize(tempCross1, tempCross1);
            glMatrix.quat.rotationTo(tempLockQuat, tempXAxis, tempCross1);
            mat4fromRotationOrigin(tempLockMat, tempLockQuat, tempTransformedPivotPoint);
            glMatrix.mat4.mul(node.matrix, tempLockMat, node.matrix);
        }
        for (var _i = 0, _a = node.childs; _i < _a.length; _i++) {
            var child = _a[_i];
    ...
    ModelRenderer.prototype.setLayerProps = function (layer, textureID) {
        ...
        if (typeof layer.TVertexAnimId === 'number') {
            ...
            glMatrix.mat4.fromRotationTranslationScale(texCoordMat4, rotationRes || defaultRotation, translationRes || defaultTranslation, scalingRes || defaultScaling);
            glMatrix.mat3.set(texCoordMat3, texCoordMat4[0], texCoordMat4[1], 0, texCoordMat4[4], texCoordMat4[5], 0, texCoordMat4[12], texCoordMat4[13], 0);
            this.gl.uniformMatrix3fv(this.shaderProgramLocations.tVertexAnimUniform, false, texCoordMat3);
        }
        else {
            this.gl.uniformMatrix3fv(this.shaderProgramLocations.tVertexAnimUniform, false, identifyMat3);
...
exports.blp = blpimage;
exports.decodeBLP = decode;
exports.generateMDL = generate$1;
exports.generateMDX = generate;
exports.getBLPImageData = getImageData;
exports.model = model;
exports.parseMDL = parse$1;
exports.parseMDX = parse;
//# sourceMappingURL=war3-model.js.map
