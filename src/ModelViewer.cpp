...
void ModelViewer::initBoneMatrices(const ModelData& modelData)
{
    ...
    for (int i : boneOrder) {
        ...
        bindPoseLocal.setToIdentity();
        bindPoseLocal.translate(bone.pivot);          // 最后一步：移回枢轴点
        bindPoseLocal.translate(bone.initTranslation); // 骨骼平移
        bindPoseLocal.rotate(bone.initRotation);      // 骨骼旋转
        bindPoseLocal.scale(bone.initScale);          // 骨骼缩放
        bindPoseLocal.translate(-bone.pivot);         // 第一步：移到枢轴原点
